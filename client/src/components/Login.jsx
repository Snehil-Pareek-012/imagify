...
const Login = () => {
    ...
    const onSubmitHandler =  async(e)=>{
        ...
        try {
            if(state === 'Login'){
                ...
                const {data} = await axios.post(backendUrl + '/api/user/register', {name, mobileNo,email, password})
    ...
    return (
        <div className='fixed top-0 left-0 bottom-0 right-0 z-10 backdrop-blur-sm
            ...
            className='relative bg-white p-10 rounded-xl text-slate-500'>
                ...
                <p className='text-sm mt-4'>Welcome chief ! Please Sign in to continue</p>
            ...
            {state !== 'Login' && <div className='border px-6 py-2 flex items-center gap-2 rounded-full mt-5'>
                ...
                { state !== 'Login' &&<div className='border px-6 py-2 flex items-center gap-2 rounded-full mt-4'>
                    <img src={assets.user_icon} alt="" />
                    <input onChange={(e) => setMobileNo(e.target.value)} value={mobileNo} type="tel" className='outline-none text-sm' placeholder='Mobile No.' required />
                </div>}
                ...
                <button className='bg-blue-600 w-full text-white py-2 rounded-full'>
                    ...
                    {state === 'Login' ? <p className='text-center mt-5'>Don't have an account ?  
                        <span className='text-blue-600 cursor-pointer'onClick={() => setState(
                            'Sign Up')}>Sign Up</span>
                    </p>
                    :
